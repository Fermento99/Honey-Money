...
function validate(username, pass, pass2) {
  ...
  document.getElementById('register-button').addEventListener('click', () => {
    ...
    console.log('fetch');
    fetch("http://localhost:3000/forgotten_password", {
      method: "POST",
      ...
      if (res.status == '200') location.href = 'http://localhost:3000/pass_successful';
      else throw Error(res.status); 
    }).catch(err => {
      console.log(err);
      document.getElementById('register-error').innerText = 'Wrong username';
    });
  });